...
export const searchDFSSUBMENU = ({ data, cond, childPathKey = "submenu" }) => {
  let final = null;
  let parentPath = [];
  let parent = null;
  let next = null;
  ...
  console.info("searchDFSSUBMENU",data);
  recursiveFind(data);
  return {
    parent,
    item: final,
...
export const createSubMenu = ({ name }) => ({ name, type: "subMenu" });
export const createMenu = ({ name }) => ({ name, type: "Menu", files: [] });
